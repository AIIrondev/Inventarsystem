#!/bin/bash
# filepath: /home/max/Dokumente/repos/Inventarsystem/restore.sh
...
restore_db() {
    ...
    if [ -f "$compressed_backup" ]; then
        ...
        sudo tar -xzf "$compressed_backup" -C "$temp_dir" "Inventarsystem-$backup_date/mongodb_backup" || {
            log_message "ERROR: Failed to extract database files from backup"
            sudo rm -rf "$temp_dir"
            return 1
        }
    ...
    csv_count=$(find "$db_backup_location" -name "*.csv" | wc -l)
...
PYTHON_SCRIPT
    ...
    python3 "$restore_script" --uri "mongodb://localhost:27017/" --db "Inventarsystem" --dir "$db_backup_location" || {
...
# Final status report
if [ $FILE_RESTORE_STATUS -eq 0 ] && [ $DB_RESTORE_STATUS -eq 0 ] && [ $RESTART_STATUS -eq 0 ]; then
